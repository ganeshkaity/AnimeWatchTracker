...
import { ChevronLeft, Youtube, HardDrive } from 'lucide-react';
import YtDlpPlayer from '../components/player/YtDlpPlayer';
import { getLocalAnime } from '../utils/localStore';

export default function YtDlpPlayerContainer({
  animeId,
  episodeId,
  episodes = [],
  onBack,
      ...
      <header className="h-14 px-4 md:px-6 bg-[#0c101c]/80 backdrop-blur-md border-b border-white/5 flex items-center justify-between z-10 shrink-0 sticky top-0">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-lg bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 text-[10px] font-black uppercase tracking-wider flex items-center gap-1">
              <Youtube size={11} className="text-cyan-400" />
              YT-DLP Player
            </span>
